...
class MeITYScraper(BaseScraper):
    """Scraper for Ministry of Electronics and IT schemes"""
    SOURCE = "MeITY"
    URL = "https://meity.gov.in/schemes"
    ...
    def scrape(self):
        ...
        cards = soup.select(".scheme-card, .card, .scheme-item, .views-row")
        ...
        for i, card in enumerate(cards[:30]):
            try:
                ...
                if href.startswith("/"):
                    link = f"https://meity.gov.in{href}"
                ...
                results.append({
                    ...
                    "source_link": link,
                    "tags": "government,india,technology,digital",
